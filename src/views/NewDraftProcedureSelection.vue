<template>
  <ion-page>
    <ion-content :scroll-y="false">
      ...
      <!-- </ion-grid> -->
      <ion-modal 
        v-if="$route.path === '/newDraftProcedureSelection' || $route.path === '/newDraftProceduresList'"
        :scroll-y="false" 
        ref="modalProcedures"
        :is-open="selectedProcedures.length > 0 && openModal" 
        ...
        >
          ...
          <transition name="fade">
            <ion-list v-if="!dragMessage" class="ion-margin" style="border-radius: 0.5rem;">
              ...
              >
                <ion-label @click="clkSelectedItem(item.procedureId)">
                  <h2>{{i + 1 + '.  ' + item.procedureData.label }}</h2>
                  <p>{{ item.procedureData.description }}</p>
                </ion-label>
                <ion-button
                size="small"
                  fill="clear"
                  mode="ios"
                  @click="clkDeleteProcedureSelection(i)"
                >
...
export default {
  name: "newDraftProcedureSelection",
  components: {
    HeaderComponent
  },
  data() {
  ...
  watch: {
    $route(to) {
      if (to.path === '/newDraftProcedureSelection') {
        this.modalOpen()
  ...
  methods: {
    async clkBodyPart (item) {
      if (this.bodyConfig === 'procedures') await this.clkNewProcedureDraft(item)
      else if (this.bodyConfig === 'complaints') await this.newComplaint(item)
    },
    async newComplaint(item) {
      if (item.bodyBlankId === "642eafc38c3e902328067a55") {
        this.view = 'face'
      } else {
        const _id = item._id
        let extra = ''
    ...
    async clkNewProcedureDraft(item){
      if (item.bodyBlankId === "642eafc38c3e902328067a55") {
        this.view = 'face'
      } else {
        const _id = item._id
        let extra = ''
        if (this.opportunityData) extra = `&opportunityId=${this.opportunityData._id}`
        this.$router.push('/newDraftProceduresList?_id=' + _id + extra)
      }
    },
    async clkDirection(){
    ...
    getActiveOpportunitiesFromUser(){
      ...
      useFetch(opt).then(async r => {
        ...
        this.gender = this.$route.query.gender ? this.$route.query.gender : this.opportunityData.opportunityData.gender
        await this.getBodyParts('body')
    ...
    async getBodyParts (type) {
      if (this.bodyConfig === 'procedures') await this.getCompanyBody(type)
      else if (this.bodyConfig === 'complaints') await this.getComplaintsBody(type)
    },
    ...
    async checkSelectedProcedures() {
      if (this.opportunityData) {
        console.log('pegou do banco')
        this.selectedProcedures = this.opportunityData.opportunityData.procedures
        window.localStorage.removeItem('proceduresArray')
      } else if (window.localStorage.getItem('proceduresArray')) {
        console.log('pegou do local storage')
        this.selectedProcedures = JSON.parse(window.localStorage.getItem('proceduresArray'))
      } else {
        this.selectedProcedures = []
      }
      this.selectedProcedures.forEach(procedure => {
        this.arrayBodies.forEach((bodyPart, i) => {
          if (procedure.bodyCompanyId === bodyPart._id){
            this.arrayBodies[i].paint = true
          }
        })
      });
      this.selectedProcedures.forEach(procedure => {
        this.arrayFace.forEach((bodyPart, i) => {
          if (procedure.bodyCompanyId === bodyPart._id){
            this.arrayFace[i].paint = true
          }
        })
      });
      
    },
    clkDeleteProcedureSelection (item) {
      if (this.opportunityData) {
        this.opportunityData.opportunityData.procedures.splice(item,1)
        const opt = {
          route: '/mobile/sales/updateProcedureFromOpportunity',
          body: {
            opportunityId: this.opportunityData._id,
            newProceduresArray: this.opportunityData.opportunityData.procedures
          }
        }
        useFetch(opt).then(r => {
          this.getActiveOpportunitiesFromUser()
        ...
        if (window.localStorage.getItem('proceduresArray')) {
          this.selectedProcedures.splice(item,1)
          window.localStorage.setItem('proceduresArray', JSON.stringify(this.selectedProcedures))
        }
      }
    },
    clkSelectedItem(id) {
      this.$refs.modalProcedures.$el.setCurrentBreakpoint(0.1)
      this.dragMessage = true;
      this.modalBackdropBreakpoint = 0.1
    ...
    //   }
    //   useFetch(opt).then(r => {
    //     const procedure = r.data
    //     console.log('o procedur adtata',procedure)
    //     return procedure
    //   })
    // },
    brekpointChanged () {
      if (this.selectedProcedures.length > 0 && this.$refs.modalProcedures) {
        this.$refs.modalProcedures.$el.getCurrentBreakpoint().then(breakpoint => {
          if (breakpoint === 0.99) {
    ...
    modalClosed() {
      if (this.$route.path === '/newDraftProcedureSelection') {
        this.dragMessage = false;
        this.modalBackdropBreakpoint = 0.1
        this.$refs.selecionados.classList.remove('big-text')   
        this.$refs.selecionados.classList.add('small-text')   
    ...
    newOpportunity () {
      const opt = {
        route: '/mobile/sales/newOrEditOpportunityProcedures',
        body: {
